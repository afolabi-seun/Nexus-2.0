...
- [ ] 1. Add DatabaseSchema property to all AppSettings and standardize UtilityService env vars
  - [x] 1.1 Add `DatabaseSchema` property to SecurityService AppSettings
  ...
  - [x] 1.2 Add `DatabaseSchema` property to ProfileService AppSettings
    - Add `public string? DatabaseSchema { get; set; }` property
    - Add `DatabaseSchema = Environment.GetEnvironmentVariable("DATABASE_SCHEMA")` to `FromEnvironment()`
    - _Requirements: 4.2, 4.6_

  - [x] 1.3 Add `DatabaseSchema` property to BillingService AppSettings
    - Add `public string? DatabaseSchema { get; set; }` property
    - Add `DatabaseSchema = Environment.GetEnvironmentVariable("DATABASE_SCHEMA")` to `FromEnvironment()`
    - _Requirements: 4.3, 4.6_

  - [x] 1.4 Add `DatabaseSchema` property to WorkService AppSettings
    - Add `public string? DatabaseSchema { get; set; }` property
    - Add `DatabaseSchema = Environment.GetEnvironmentVariable("DATABASE_SCHEMA")` to `FromEnvironment()`
    - _Requirements: 4.4, 4.6_

  - [x] 1.5 Add `DatabaseSchema` property and standardize env var names in UtilityService AppSettings
    - Add `public string? DatabaseSchema { get; set; }` property
    - Add `DatabaseSchema = Environment.GetEnvironmentVariable("DATABASE_SCHEMA")` to `FromEnvironment()`
...
- [ ] 2. Add conditional HasDefaultSchema and InMemory detection to all DbContexts
  - [x] 2.1 Update SecurityDbContext with schema support
  ...
  - [x] 2.2 Update ProfileDbContext with schema support
    - Add `private readonly string? _databaseSchema` field and constructor parameter `string? databaseSchema = null`
  ...
  - [x] 2.3 Update BillingDbContext with schema support
    - Add `private readonly string? _databaseSchema` field and constructor parameter `string? databaseSchema = null`
    - In `OnModelCreating`, add the `HasDefaultSchema` + `IsInMemory()` guard before `base.OnModelCreating(modelBuilder)`
    - _Requirements: 2.3, 2.6, 9.1, 9.3_

  - [x] 2.4 Update WorkDbContext with schema support
    - Add `private readonly string? _databaseSchema` field and constructor parameter `string? databaseSchema = null`
    - In `OnModelCreating`, add the `HasDefaultSchema` + `IsInMemory()` guard before `base.OnModelCreating(modelBuilder)`
    - _Requirements: 2.4, 2.6, 9.1, 9.3_

  - [x] 2.5 Update UtilityDbContext with schema support
    - Add `private readonly string? _databaseSchema` field and constructor parameter `string? databaseSchema = null`
...
- [ ] 3. Update DI registration with MigrationsHistoryTable in all services
  - [x] 3.1 Update SecurityService DependencyInjection.cs
  ...
  - [x] 3.2 Update ProfileService DependencyInjection.cs
    - Same pattern: add `MigrationsHistoryTable` configuration and pass schema to DbContext
    - _Requirements: 5.1, 5.2, 5.3_

  - [x] 3.3 Update BillingService DependencyInjection.cs
    - Same pattern: add `MigrationsHistoryTable` configuration and pass schema to DbContext
    - _Requirements: 5.1, 5.2, 5.3_

  - [x] 3.4 Update WorkService DependencyInjection.cs
    - Same pattern: add `MigrationsHistoryTable` configuration and pass schema to DbContext
    - _Requirements: 5.1, 5.2, 5.3_

  - [x] 3.5 Update UtilityService DependencyInjection.cs
    - Same pattern: add `MigrationsHistoryTable` configuration and pass schema to DbContext
    - _Requirements: 5.1, 5.2, 5.3_

- [ ] 4. Create or update DesignTimeDbContextFactory for all services
  - [x] 4.1 Create DesignTimeDbContextFactory for SecurityService
...
- [x] 5. Checkpoint — Verify code compiles
  - Ensure all projects build successfully with `dotnet build Nexus-2.0.sln`
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Update environment configuration files for all services
  - [x] 6.1 Update SecurityService environment files
  ...
  - [x] 6.2 Update ProfileService environment files
  ...
  - [x] 6.3 Update BillingService environment files
  ...
  - [x] 6.4 Update WorkService environment files
  ...
  - [x] 6.5 Update UtilityService environment files (with env var rename)
...
- [x] 7. Update Docker init script and docker-compose files
  - [x] 7.1 Replace `docker/init-databases.sql` with shared database + schemas
  ...
  - [x] 7.2 Update `docker/docker-compose.yml`
  ...
  - [x] 7.3 Update `docker/docker-compose.local.yml`
    - Same changes as 7.2 but with `host.docker.internal` host
    - _Requirements: 10.4, 7.6_

  - [x] 7.4 Update `docker/docker-compose.server.yml`
    - Same changes as 7.2 but with `host.docker.internal` host
    - _Requirements: 10.4, 7.6_
